...
def record_add(request):
    ...
    return render(
        request, "finance/record_form.html", {"form": form, "title": "Добавить запись"}
    )
...
def edit_reference_item(request, model_name, pk):
    ...
    Form = modelform_factory(
        ...
        fields=(
            ...
            else ("name", "category") if model_name == "subcategory" else ("name",)
        ),
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, f"{model._meta.verbose_name.capitalize()} успешно обновлен(а)."
            )
            return redirect("reference_manage")
    else:
